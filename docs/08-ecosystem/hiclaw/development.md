...
> [← 上一步：导入 Worker](import-worker.md) · [📖 教程目录](README.md) · [下一步：常见问题 →](faq.md)
...
HiClaw 项目代码库结构包含 `manager/`（Manager Agent）、`worker/`（Worker Agent）、`controller/`（基础设施控制面）、`helm/`（Helm Chart）等模块。
