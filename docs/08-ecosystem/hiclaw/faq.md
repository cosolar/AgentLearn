...
> [← 上一步：开发指南](development.md) · [📖 教程目录](README.md)
...
这种方式仅对当前会话生效，重启后会恢复为主用模型。且仅支持预置的已知模型列表中的模型，完整列表见 [`manager/configs/known-models.json`](https://github.com/higress-group/hiclaw/tree/main/manager/configs/known-models.json)。
...
仅对当前会话生效，重启后恢复为主用模型。仅支持预置的已知模型，完整列表见 [`manager/configs/known-models.json`](https://github.com/higress-group/hiclaw/tree/main/manager/configs/known-models.json)。
